...
async def update_template(tpl_id: str, tpl_update: TemplateUpdate):
    coll = db_manager.get_collection("label_templates")
    existing = await coll.find_one({"$or": [{"id": tpl_id}, {"_id": tpl_id}]})
    if not existing:
    ...
    await coll.update_one({"$or": [{"id": tpl_id}, {"_id": tpl_id}]}, {"$set": update_data})
    updated_doc = await coll.find_one({"$or": [{"id": tpl_id}, {"_id": tpl_id}]})
